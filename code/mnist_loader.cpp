...
void mnist_loader::read_mnist(std::vector<std::vector<double> >& vectors){
    vectors.clear();
...
void mnist_loader::read_mnist(std::vector<cv::Mat>& matrices){
    matrices.clear();
...
void mnist_loader::read_mnist(std::vector<std::vector<double> >& vectors, std::vector<cv::Mat>& matrices){
    vectors.clear();
    matrices.clear();
    vectors.reserve(m_image_number);
    matrices.reserve(m_image_number);
    for(size_t i=0; i<m_image_number; ++i){
        vectors.push_back(generate_vector());
        m_fin.seekg((size_t)m_fin.tellg()-sizeof(unsigned char)*m_image_size);
        matrices.push_back(generate_matrix());
    }
    m_fin.seekg(m_offset);
}
    
void mnist_loader::iterate_mnist(std::vector<double>& vector){
...
void mnist_loader::iterate_mnist(cv::Mat& matrix){
    if(m_image_counter==m_image_number)
        return;
    m_fin.seekg(m_image_counter*sizeof(unsigned char)*m_image_size+m_offset);
...
void mnist_loader::iterate_mnist(std::vector<double>& vector, cv::Mat& matrix){
    if(m_image_counter==m_image_number)
        return;
    m_fin.seekg(m_image_counter*sizeof(unsigned char)*m_image_size+m_offset);
    vector=generate_vector();
...
cv::Mat mnist_loader::rotate(const cv::Mat& matrix, double angle){
...
cv::Mat mnist_loader::to_matrix(const std::vector<double>& vector, size_t rows, size_t cols){
    cv::Mat result(rows, cols, CV_8UC1);
    memcpy(result.data, vector.data(), vector.size()*sizeof(double));
    return result;
}

std::vector<double> mnist_loader::to_vector(const cv::Mat& matrix){
...
std::vector<double> mnist_loader::generate_vector(){
        std::vector<double> result;
...
cv::Mat mnist_loader::generate_matrix(){
